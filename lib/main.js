...
module.exports = {
  ...
  activate() {
    ...
    this.disposables = new CompositeDisposable(
      ...
      atom.commands.add("atom-workspace", {
        "toggle:open-config": () => this.openConfig(),
      }),
  ...
  toggle(editorElement, where) {
    if (!this.userWordGroup) this.userWordGroup = readCSON(this.userConfigPath)

    const editor = editorElement.getModel()
    editor.transact(() => editor.getCursors().forEach(cursor => this.toggleWord(cursor, where)))
  },
  ...
  toggleWord(cursor, where) {
    ...
    editor.scanInBufferRange(/\b\w+\b/g, scanRange, ({range, replace, matchText, stop}) => {
      ...
      if (where === "here" && !range.containsPoint(cursorPosition)) {
        stop()
        return
      }
  ...
  async openConfig() {
    const editor = await atom.workspace.open(this.userConfigPath, {searchAllPanes: true})
